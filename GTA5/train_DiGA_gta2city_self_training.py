...
gta5_data_aug = Compose([RandomHorizontallyFlip(),
                         RandomSized_and_Crop([256, 256])
                         ])

city_data_aug = Compose_Pseudo([RandomHorizontallyFlip_Pseudo(),
                         RandomSized_and_Crop_Pseudo([256, 256])
                        ])
...
class_features = Class_Features(numbers=4)
...
upsample_src = nn.Upsample(size=[256, 256], mode='bilinear', align_corners=True)
upsample_tgt = nn.Upsample(size=[256, 256], mode='bilinear', align_corners=True)
...
for i_iter in range(num_steps):
    ...
    if i_iter % num_calmIoU == 0 and i_iter>0:
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(val_loader)):
            ...
            image_ds = nn.functional.interpolate(images_val, (256, 256), mode='bilinear', align_corners=True)
            with torch.no_grad():
